export AverageBoostConverterInitialState,
       AverageBoostConverterStudy,
       AverageBoostConverterTrace

const AverageBoostConverterInitialState = DCDCConverterInitialState

struct AverageBoostConverterStudy{T<:BridgeTopologyDescriptor}
    ...
    function AverageBoostConverterStudy(
        specification::ConverterSystemSpecification;
        ...
        selection.family === BoostChopper || throw(ArgumentError(
            "average boost study requires the canonical boost-chopper family",
        ))
        selection.fidelity === AverageValue || throw(ArgumentError(
            "average boost study requires explicit AverageValue fidelity",
        ))
        selection.application === StandaloneConversion || throw(ArgumentError(
            "average boost study is a standalone converter, not an application composition",
        ))
        topology.family === :step_up_chopper || throw(ArgumentError(
            "average boost study requires the canonical B200 step-up chopper topology",
        ))
        specification.topology_signatures == (bridge_topology_signature(topology),) ||
            throw(ArgumentError(
                "average boost specification does not bind the supplied B200 topology",
            ))
        converter_system_is_ready(converter_system_readiness(specification)) ||
            throw(ArgumentError("average boost converter specification is not ready"))
        initial_state.inductor_current_a >= 0.0 && initial_state.output_voltage_v >= 0.0 ||
            throw(ArgumentError(
                "average boost initial current and voltage must follow nonnegative references",
            ))
        values = Float64.((
            input_voltage_v,
        ...
        all(isfinite, values) || throw(ArgumentError(
            "average boost study parameters must be finite",
        ))
        values[1] > 0.0 && values[2] > 0.0 && values[3] >= 0.0 &&
            all(>(0.0), values[4:6]) || throw(ArgumentError(
                "average boost voltage, source resistance, L, C, and load must be positive and inductor resistance nonnegative",
            ))
        values[7] >= 0.0 && values[8] > values[7] || throw(ArgumentError(
            "average boost stop time must follow its nonnegative start time",
        ))
        ...
        isapprox(step_count, round(step_count); atol=1.0e-10, rtol=1.0e-10) ||
            throw(ArgumentError(
                "average boost horizon must contain an integer number of fixed steps",
            ))
...
struct AverageBoostConverterTrace

...
mutable struct AverageInvertingBuckBoostConverterRuntime{S}
    study::S
...
function _average_inverting_buck_boost_state_matrix(study)
    duty = study.specification.modulation.duty
    transfer_fraction = 1.0 - duty
    series_resistance = study.inductor_resistance_ohm +
        duty * study.source_resistance_ohm
    return [
...
function _average_inverting_buck_boost_stored_energy(study, current, voltage)
    return 0.5 * study.inductance_h * current^2 +
        0.5 * study.capacitance_f * voltage^2
end

function prepare_average_inverting_buck_boost_converter(
    study::AverageInvertingBuckBoostConverterStudy,
)
    step = study.specification.timing.fixed_step_s
    state_matrix = _average_inverting_buck_boost_state_matrix(study)
    ...
    source_increment = left_matrix \ [
        step * study.specification.modulation.duty * study.input_voltage_v /
            study.inductance_h,
        0.0,
    ]
    ...
    _record_average_inverting_buck_boost_sample!(runtime, 1, 0.0, 0.0)
    return runtime
end

function _record_average_inverting_buck_boost_sample!(
    runtime,
    sample_index,
    kcl_residual_a,
    energy_residual_w,
    ...
    runtime.input_current_trace_a[sample_index] = duty * current
    ...
    runtime.stored_energy_trace_j[sample_index] =
        _average_inverting_buck_boost_stored_energy(study, current, voltage)
...
function _advance_average_inverting_buck_boost_converter!(runtime, sample_index)
    study = runtime.study
    step = study.specification.timing.fixed_step_s
    duty = study.specification.modulation.duty
    transfer_fraction = 1.0 - duty
    ...
    previous_energy = _average_inverting_buck_boost_stored_energy(
        study,
        previous_current,
        previous_voltage,
    )
    ...
    runtime.inductor_current_a >= 0.0 && runtime.output_voltage_v <= 0.0 ||
        throw(DomainError(
            ...
            "average inverting buck-boost state left its signed continuous-conduction domain",
        ))
    current = runtime.inductor_current_a
    voltage = runtime.output_voltage_v
    runtime.time_s = study.start_time_s + (sample_index - 1) * step
    input_energy = 0.5 * step * duty * study.input_voltage_v *
    ...
    stored_energy = _average_inverting_buck_boost_stored_energy(study, current, voltage)
    energy_residual = (
        input_energy - load_energy - loss_energy - (stored_energy - previous_energy)
    ) / step
    average_current = 0.5 * (previous_current + current)
    average_voltage = 0.5 * (previous_voltage + voltage)
    capacitor_current = study.capacitance_f * (voltage - previous_voltage) / step
    kcl_residual = transfer_fraction * average_current +
        average_voltage / study.load_resistance_ohm + capacitor_current
    runtime.dissipated_energy_j += loss_energy
    return _record_average_inverting_buck_boost_sample!(
        runtime,
        sample_index,
        kcl_residual,
        energy_residual,
...
function _average_inverting_buck_boost_result_state(runtime)
    study = runtime.study
    duty = study.specification.modulation.duty
    current = runtime.inductor_current_a
    voltage = runtime.output_voltage_v
    ...
    return ConverterSystems.ConverterSystemState(
        ...
        [duty * current, voltage / study.load_resistance_ohm],
        BitVector(),
        BitVector(),
        BitVector(),
...
function execute_average_inverting_buck_boost_converter!(
    runtime::AverageInvertingBuckBoostConverterRuntime,
)
    ...
    result = ConverterSystems.converter_system_result(
        ...
        _average_inverting_buck_boost_result_state(runtime);
        accepted=true,
        status=:ok,
    ...
    return ConverterSystems.AverageInvertingBuckBoostConverterTrace(
